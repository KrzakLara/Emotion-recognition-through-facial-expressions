...
class F1ScoreCallback(tf.keras.callbacks.Callback):
    ...
    def on_train_end(self, logs=None):
        ...
        print("Validation True Shape: ", y_true.shape)
        print("Validation Pred Probs Shape: ", y_pred_probs.shape)
...
def main():
    try:
        ...
        val_pred = np.concatenate([model.predict(x) for x, _ in validation_dataset])
        val_true = np.concatenate([y for _, y in validation_dataset])

        print("Validation True Shape: ", val_true.shape)
        print("Validation Pred Shape: ", val_pred.shape)

        # Ensure y_true is in the correct shape for precision_recall_curve
        if val_true.ndim == 2 and val_true.shape[1] == 1:
            val_true = val_true.flatten()
        if val_pred.ndim == 2 and val_pred.shape[1] == 1:
            val_pred = val_pred.flatten()

        # Plot precision-recall curve and confusion matrix
        plot_precision_recall(val_true, val_pred, class_names)
        plot_confusion_matrix(np.argmax(val_true, axis=1), np.argmax(val_pred, axis=1), class_names)

        # Print classification report
        print(classification_report(np.argmax(val_true, axis=1), np.argmax(val_pred, axis=1), target_names=class_names))
